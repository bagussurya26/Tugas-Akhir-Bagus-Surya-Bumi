    ...
    <div id="custom_styles" class="col-lg-12 layout-spacing col-md-12">
        <div class="statbox widget box box-shadow">
            ...
            <div class="widget-content widget-content-area">
                <form method="POST" action="{{ route('bulanan.proses') }}">
                    @csrf
                    <div class="row layout-spacing">
                        <div class="col-3">
                            <label class="form-label">Target Bulan <small
                                    class="text-muted ms-2 pb-1">(Required)</small></label>
                            <select class="form-select" name="target_bulan" id="target_bulan" oninput="getBulan()"
                                required>
                        ...
                        <div class="col-3">
                            <label class="form-label">Mulai Data Dari Bulan <small
                                    class="text-muted ms-2 pb-1">(Required)</small></label>
                            <select class="form-select" name="data_bulan" id="data_bulan" required>
                                <option selected disabled value="">Choose...</option>
                            </select>
                        </div>
...
@if(session()->has('bulanan'))
@php
$bulanans = session('bulanan');
@endphp

@foreach ($bulanans as $index => $bulanan)
<div class="row">
    <div id="chartColumn" class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
        <div class="statbox widget box box-shadow">
            <div class="widget-header">
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <h4>Hasil Peramalan Produk {{ $bulanan['jenis']=='kategori' ? 'Kategori' : '' }} <b>{{
                                $index }}</b> - Bulanan <b>{{
                                end($bulanan['waktuDisplay']) }}</b>
                        </h4>
                    </div>
                </div>
            </div>
            <div class="widget-content widget-content-area">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <p>Hasil peramalan produk {{ $bulanan['jenis']=='kategori' ? 'Kategori' : '' }} <b>{{ $index
                            }}</b> untuk Bulan <b>{{
                            end($bulanan['waktuDisplay']) }}</b> dengan
                        acuan data penjualan Bulan <b>{{ reset($bulanan['waktuDisplay']) }}</b> adalah
                        sejumlah <b>{{ end($bulanan['dataWMA']) }} Pcs</b> dengan
                        persentase kesalahan peramalan sebesar <b>
                            @php
                            $lastMAPE = end($bulanan['dataMAPE']);
                        ...
                        dinyatakan <b>{{ $bulanan['akurasi'] }} ({{ $displayMAPE }})</b></p>
                </div>
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <p id="result"></p>
                </div>
                <div id="peramalan-{{ $bulanan['id'] }}" class=""></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var bulanan = @json(session('bulanan'));

    Object.keys(bulanan).forEach(function (category) {
        
        let categoryData = bulanan[category];
